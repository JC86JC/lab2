...
```{r}
for (i in 1:100) {
  if (senators$objects[[i]]$state == 'NC') {
    print(
      paste('The birthday for senator',
                senators$objects[[i]]$person$name,
                'is',
                senators$objects[[i]]$person$birthday ))
  }
}
```
...
```{r}
for ( i in 1:100){
  if(is.null(senators$objects[[i]]$leadership_title) == FALSE){
    print(senators$objects[[i]]$person$twitterid)
  }
}
```
